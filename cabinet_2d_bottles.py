...
def bottle_svg(bottle_id: str, mode: str = "gaslight", selected: bool = False,
               uv_mode: bool = False, size: int = 200) -> str:
    ...
    inner_svg = renderer(bottle_id, p, w, h, uv_mode)

    uv_attr = 'filter="url(#uvglow)"' if uv_mode else ""
...
def _chloroform(bid, p, w, h, uv):
    ...
    cx = w // 2
    return f'''
    <!-- Body: wide round bottom, narrow neck -->
...
def _dovers_powder(bid, p, w, h, uv):
    ...
    cx = w // 2
    return f'''
    <!-- Body: standard tall brown bottle -->
    ...
    <!-- Dosage — crossed out and rewritten -->
...
def _paregoric(bid, p, w, h, uv):
    ...
    <!-- Sleeping child illustration (simplified) -->
...
def _smelling_salts(bid, p, w, h, uv):
    ...
    <!-- White crystals inside (not liquid) -->
